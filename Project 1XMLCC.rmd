...
* **The function is satedeath calculates the measurement of death and age adjusted death rate by state**

```{r function}
knitr::opts_chunk$set(echo = FALSE, eval = TRUE, warning=T, message = T)

HealthData$year<-as.numeric(as.character(HealthData$year))
HealthData$state<-as.character(HealthData$state)


yeardeath <- function (year ) {
  data <- HealthData %>% filter(year == year) %>% select(deaths,aadr)
  kable(apply(data, 2, summary), caption = paste("Summary of death and age adjusted death in ", year), 
        digit = 0)
}
yeardeath(2016)
yeardeath(2010)
